...
surat=$(curl -sS https://raw.githubusercontent.com/halonthe/permission/main/ip | grep -w $ipes | awk '{print $4}')
...
EOF
    ...
    fun_bot1() {
      [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
        wget -qO- https://raw.githubusercontent.com/halonthe/bot_panel/main/BotAPI.sh >/etc/.maAsiss/.Shellbtsss
      }
